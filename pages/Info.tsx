...
import { Snowflake, AlertTriangle, ShieldAlert, ChevronDown, ChevronUp, Phone } from 'lucide-react';
    ...
    <div className="pb-24 space-y-4 animate-fade-in pt-4">
      ...
      {sections.map((section) => (
        <div key={section.id} className="relative">
          <GlassCard 
            className={`transition-all duration-300 !p-0 overflow-hidden ${section.borderColor} ${openSection === section.id ? 'bg-white/15' : 'hover:bg-white/10'}`}
          >
            <button 
            ...
            <div 
              className={`transition-all duration-300 ease-in-out px-5 overflow-hidden ${
                openSection === section.id ? 'max-h-[500px] opacity-100 pb-5' : 'max-h-0 opacity-0'
              }`}
            >
