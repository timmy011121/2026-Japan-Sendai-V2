...
import { MapPin, Clock, Info, Utensils, ShoppingBag, Camera, ChevronDown, ChevronUp, Wallet, Star, NotepadText, Car, TrainFront, LocateFixed, ExternalLink, Map as MapIcon } from 'lucide-react';

export const ItineraryPage: React.FC = () => {
  const [selectedDayIndex, setSelectedDayIndex] = useState(0);
  const currentDay = TRIP_DATA[selectedDayIndex];
  
  // State for map interactivity
  const [activeMapLocation, setActiveMapLocation] = useState<string>(currentDay.hotel.name);

  // Reset map location when day changes
  useEffect(() => {
    setActiveMapLocation(currentDay.hotel.name);
  }, [selectedDayIndex, currentDay.hotel.name]);

  // Logic to separate primary food spot vs backups
  const primarySpots: Spot[] = [];
  const backupSpots: Spot[] = [];
  let foundFirstFood = false;

  currentDay.spots.forEach(spot => {
    const isFood = spot.type === SpotType.RESTAURANT || spot.type === SpotType.DESSERT;
  ...
  primarySpots.sort((a, b) => a.time.localeCompare(b.time));

  return (
    <div className="pb-24 animate-fade-in h-full flex flex-col">
      {/* Date Selector - Sticky Top */}
        ...
        <div className="flex overflow-x-auto no-scrollbar space-x-3 py-2 px-1">
          {TRIP_DATA.map((day, idx) => (
            <button
              ...
              className={`
                flex flex-col items-center justify-center min-w-[4.5rem] py-2.5 px-1 rounded-2xl border transition-all duration-300 active:scale-95
                ${selectedDayIndex === idx 
                  ? 'bg-cyan-500/20 border-cyan-400/50 shadow-[0_0_20px_rgba(6,182,212,0.2)]' 
      ...
      {/* Simplified Weather Widget + Interactive Map Container */}
      <div className="space-y-4 mb-6 mt-2">
          
          {/* Weather Compact */}
          <GlassCard className="flex items-center justify-between !py-4 !px-6 bg-gradient-to-r from-blue-950/60 to-white/[0.05] border-white/10">
             ...
             <div className="text-right">
                ...
                <span className="text-[10px] text-white/40 flex items-center justify-end gap-1 mt-1 font-medium bg-black/20 px-2 py-0.5 rounded-full w-fit ml-auto">
                   <MapPin size={10} /> {currentDay.hotel.address.split(' ')[0] || 'Tohoku'}
                </span>
             </div>
          </GlassCard>
          ...
          <GlassCard className="!p-0 overflow-hidden">
             <div className="h-56 w-full relative bg-[#1a1a1a]">
               ...
               {/* Google Maps Embed iframe with Dark Mode Filter */}
               <iframe
                 title="Map"
                 width="100%"
                 height="100%"
             ...
             <div className="p-4 border-t border-white/10">
                ...
                <div className="flex overflow-x-auto no-scrollbar gap-2 pb-1">
                   <button 
                      ...
                      className={`
                        shrink-0 whitespace-nowrap px-3 py-2 rounded-xl text-xs font-medium transition-all duration-300 border
                        ${activeMapLocation === currentDay.hotel.name 
      ...
      <div className="relative pl-4 space-y-8">
        ...
        {primarySpots.map((spot, index) => (
           <SpotCard key={spot.id} spot={spot} isFirstFood={spot.type === SpotType.RESTAURANT && index < 3} />
        ))}
      </div>
        ...
        <div className="mt-12 pl-4 pt-6 border-t border-dashed border-white/10">
          ...
          <div className="space-y-4">
            {backupSpots.map((spot) => (
              <SpotCard key={spot.id} spot={spot} isBackup />
            ))}
          </div>
        </div>
      )}
...
const SpotCard: React.FC<{ spot: Spot; isFirstFood?: boolean; isBackup?: boolean }> = ({ spot, isFirstFood, isBackup }) => {
  const [isExpanded, setIsExpanded] = useState(false);

  const getIcon = () => {
    switch (spot.type) {
      case SpotType.RESTAURANT: return <Utensils size={18} />;
      case SpotType.DESSERT: return <Utensils size={18} />;
      case SpotType.SHOPPING: return <ShoppingBag size={18} />;
      case SpotType.TRANSPORT: 
        if (spot.name.includes('車') || spot.name.includes('開')) return <Car size={18} />;
        if (spot.name.includes('新幹線') || spot.name.includes('鐵')) return <TrainFront size={18} />;
  ...
  const getTheme = () => {
    ...
    if (spot.type === SpotType.RESTAURANT || spot.type === SpotType.DESSERT) {
       return {
  ...
  return (
    <div className={`relative flex gap-4 ${isBackup ? 'opacity-80 scale-95 origin-left' : ''}`}>
      {!isBackup && (
        <div className={`
          z-10 mt-6 min-w-[20px] h-[20px] rounded-full border-4 border-[#0a0a0a] 
      ...
      <GlassCard 
        className={`
          flex-1 transition-all duration-300 cursor-pointer group
          ${theme.border} 
          ${isExpanded ? theme.bgActive : theme.bgHover}
        `}
      ...
      >
        ...
        {spot.tags && spot.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-2.5">
            {spot.tags.map((tag, i) => {
              const isHighlight = tag.includes('必') || tag.includes('重要') || tag.includes('預約');
              return (
